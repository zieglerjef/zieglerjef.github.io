...
<details><summary> Abstract </summary>
<br/><p>
Remote workers hired for surveys and experiments are often inattentive, which can hinder researchers’ ability to draw substantive or causal inferences. As such, most practitioners include multiple factual or informational manipulation checks to remove poor-quality respondents, but eliminating inattentive participants from the sample based on their inability to pass manipulation checks may cause researcher-induced bias. In response to these shortcomings, I develop an automatic and standardized methodology to measure attention that relies on the text provided by respondents from open-ended manipulation checks. There are multiple benefits of this approach: first, it reduces the need for subjective, paid humans to analyze open-ended text and it does not require researchers to drop participants that would fail a traditional manipulation check. Additionally, I outline guidelines to diagnose the impact of inattentive online workers, including how to re-estimate the overall results varying which respondents are included in the sample based on attention, and how to assess the average treatment effect of those respondents that would and would not pass the manipulation check.
</p><br/>
</details>
<br/>	
